...
public class HealthComponent : Node2D {
	...
	DamagePrint MyDamagePrint { get { return GetNode<DamagePrint> ("DamagePrint"); } }
	...
	public float TakeDamage (AttackResource attackData, Vector2 direction) {
		if (isInvulnerable)
			return 0;
		float damage = (armor == null) ? attackData.Damage : armor.ApplyDamage (attackData.Types, attackData.Damage);
		HP -= damage;
		MyDamagePrint.ShowDamage (damage);
